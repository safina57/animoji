...
export default function UnifiedInput({ mode, onModeToggle }: UnifiedInputProps) {
  ...
  async function generate() {
    ...
    if (isAnime) {
      if (isRefinement) {
        if (!animeJobId) { dispatch(failGeneration("No active job to refine")); return; }
        dispatch(startGeneration());
        try {
          await submitRefinement(animeJobId, prompt);
        } catch (err) {
          dispatch(failGeneration(err instanceof Error ? err.message : "Failed to submit refinement"));
        }
      } else {
        if (!referenceImage) { dispatch(failGeneration("Please upload an image first")); return; }
        dispatch(startGeneration());
        try {
          const res = await submitJob(referenceImage, prompt);
          dispatch(setJobId(res.job_id));
        } catch (err) {
          dispatch(failGeneration(err instanceof Error ? err.message : "Something went wrong. Please try again."));
        }
      }
    } else {
      if (!referenceImage) { dispatch(failEmojiGeneration("Please upload an image first")); return; }
      dispatch(startEmojiGeneration());
      try {
        const res = await submitEmojiJob(referenceImage, prompt);
        dispatch(setEmojiJobId(res.job_id));
      } catch (err) {
        dispatch(failEmojiGeneration(err instanceof Error ? err.message : "Something went wrong. Please try again."));
      }
    }
  }
              ...
              <DropdownMenu>
                ...
                >
                  ...
                  <div className="relative p-2">
                    ...
                    >
                      <span className="material-symbols-outlined text-[18px] font-display text-slate-900 dark:text-white">add_photo_alternate</span>
                      <span className="font-display text-sm text-slate-900 dark:text-white truncate">Upload photo</span>
                    </DropdownMenuItem>
